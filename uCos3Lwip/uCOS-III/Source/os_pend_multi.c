...
*                                OS_ERR_PEND_ABORT        The wait on the events was aborted; check the .RdrPtr fields 
*                                                         for which objects were aborted.
*                                OS_ERR_PEND_DEL          The wait on the events was aborted; check the .RdrPtr fields 
*                                                         for which objects were aborted.
...
#if OS_CFG_Q_EN > 0u
        p_q = (OS_Q *)p_pend_data_tbl->PendObjPtr;               /* Assume we are pointing to a message queue object  */
...
#if OS_CFG_SEM_EN > 0u
        p_sem = (OS_SEM *)p_pend_data_tbl->PendObjPtr;           /* Assume we are pointing to a semaphore object      */
                                   ...
                                   OS_OBJ_QTY     tbl_size)
{
    ...
    for (i = 0u; i < tbl_size; i++) {
        ...
        ctr = 0u;
#if OS_CFG_SEM_EN  > 0u
        p_sem = (OS_SEM *)p_pend_data_tbl->PendObjPtr;           /* All objects to pend on must be of type OS_SEM ... */
...
#if OS_CFG_Q_EN > 0u
        p_q = (OS_Q *)p_pend_data_tbl->PendObjPtr;               /* ... or of type OS_Q                               */
...
#if OS_CFG_SEM_EN > 0u
        p_sem = (OS_SEM *)p_pend_data_tbl->PendObjPtr;
        if (p_sem->Type == OS_OBJ_TYPE_SEM) {
...
#if OS_CFG_Q_EN > 0u
        p_q = (OS_Q *)p_pend_data_tbl->PendObjPtr;
        if (p_q->Type == OS_OBJ_TYPE_Q) {
