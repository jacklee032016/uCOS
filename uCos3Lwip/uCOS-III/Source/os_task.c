    ...
    switch (p_tcb->TaskState) {
        ...
        case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
             switch (p_tcb->PendOn) {                       /* What to do depends on what we are pending on           */
                 case OS_TASK_PEND_ON_TASK_Q:               /* ... Nothing to do because not in a pend list           */
                 case OS_TASK_PEND_ON_TASK_SEM:
                 case OS_TASK_PEND_ON_FLAG:                 /* ... Nothing to do because not in a priority list       */
                      break;
...
*
*              p_stk_limit    is a pointer used to set a 'watermark' limit for the stack  
*
*              stk_size       is the size of the stack in number of elements.  If CPU_STKis set to CPU_INT08U,
*                             'stk_size' corresponds to the number of bytes available.  If CPU_STKis set to
...
*                                                             if OS_PREEMPTION_LOCK_METHOS is set to 1 and you tried
                    ...
                    OS_ERR        *p_err)
{
    ...
    *p_err = OS_ERR_NONE;
    ...
    p_tcb->StkSize       = stk_size;                        /* Save the stack size (in number of CPU_STKelements)     */
...
#if OS_CFG_DBG_EN > 0u
    if (OSTaskQty < (OSCfg_TaskDbgTblSize - 1u)) {    
        OSCfg_TaskDbgTbl[OSTaskQty] = p_tcb;
        p_tcb->DbgIx                = OSTaskQty;
    }
#endif
...
#if OS_CFG_DBG_EN > 0u
    OSCfg_TaskDbgTbl[p_tcb->DbgIx] = OSCfg_TaskDbgTbl[OSTaskQty];
    OSCfg_TaskDbgTbl[OSTaskQty]    = (OS_TCB *)0;
    p_tcb->DbgIx                   = OSCfg_TaskDbgTblSize;
#endif
...
*                            OS_ERR_SEM_OVF           If the post would cause the semahore count to overflow.
*
...
*
*              p_free      is a pointer to a variable that will receive the number of free 'bytes' on the task's stack.
*
*              p_used      is a pointer to a variable that will receive the number of used 'bytes' on the task's stack.
*
                    ...
                    OS_ERR       *p_err)
{
...
#if OS_CFG_ARG_CHK_EN > 0u
    if (p_free == (OS_STK_SIZE *)0) {                       /* User must specify valid destinations for the sizes     */
        *p_err = OS_ERR_PTR_INVALID;
        return;
    }
    
    if (p_used == (OS_STK_SIZE *)0) {
        *p_err = OS_ERR_PTR_INVALID;
        return;
    }
#endif
    ...
    if (p_tcb->StkPtr == (CPU_STK*)0) {                     /* Make sure task exist                                   */
        CPU_CRITICAL_EXIT();
        *p_free = (OS_STK_SIZE)0;
        *p_used = (OS_STK_SIZE)0;
...
#endif
    *p_free = free_stk                    * sizeof(CPU_STK);/* Compute number of free bytes on the stack              */
    *p_used = (p_tcb->StkSize - free_stk) * sizeof(CPU_STK);/* Compute number of bytes used on the stack              */
    *p_err  = OS_ERR_NONE;
}
#endif
...
void  OS_TaskInit (OS_ERR  *p_err)
{
    ...
    for (i = 0u; i < OSCfg_TaskDbgTblSize; i++) {
        OSCfg_TaskDbgTbl[i] = (OS_TCB *)0;
    }
#endif
...
void  OS_TaskInitTCB (OS_TCB *p_tcb)
{
    ...
    OS_MsgQInit(&p_tcb->MsgQ,
                (OS_MSG_QTY)0);                     
#if OS_CFG_TASK_PROFILE_EN > 0u
    p_tcb->MsgQPendTime       = (CPU_TS         )0;
    p_tcb->MsgQPendTimeMax    = (CPU_TS         )0;
#endif
#endif

#if OS_CFG_FLAG_EN > 0u
    p_tcb->FlagsPend          = (OS_FLAGS       )0;
    p_tcb->FlagsOpt           = (OS_OPT         )0;
    p_tcb->FlagsRdy           = (OS_FLAGS       )0;
#endif    

#if OS_CFG_TASK_REG_TBL_SIZE > 0u
    for (id = 0u; id < OS_CFG_TASK_REG_TBL_SIZE; id++) {
        p_tcb->RegTbl[id] = (OS_REG)0;
    }
#endif

    p_tcb->SemCtr             = (OS_SEM_CTR     )0;
#if OS_CFG_TASK_PROFILE_EN > 0u
    p_tcb->SemPendTime        = (CPU_TS         )0;
    p_tcb->SemPendTimeMax     = (CPU_TS         )0;
#endif

    p_tcb->StkSize            = (OS_STK_SIZE    )0;
    

#if OS_CFG_TASK_SUSPEND_EN > 0u
    p_tcb->SuspendCtr         = (OS_NESTING_CTR )0;
#endif

#if OS_CFG_STAT_TASK_STK_CHK_EN > 0u  
    p_tcb->StkFree            = (OS_STK_SIZE    )0;
    p_tcb->StkUsed            = (OS_STK_SIZE    )0;
#endif
    ...
    p_tcb->TimeQuanta         = (OS_TICK        )0;
    p_tcb->TimeQuantaCtr      = (OS_TICK        )0;

#if OS_CFG_TASK_PROFILE_EN > 0u
    p_tcb->CPUUsage           = (OS_CPU_USAGE   )0;
    p_tcb->CtxSwCtr           = (OS_CTX_SW_CTR  )0;
    p_tcb->CyclesDelta        = (CPU_TS         )0;
    ...
    p_tcb->Prio               = (OS_PRIO        )0xFFFFFFFF;

#if OS_CFG_DBG_EN > 0u
    p_tcb->DbgIx              = (OS_OBJ_QTY     )OSCfg_TaskDbgTblSize;
#endif
}
                            ...
                            OS_ERR  *p_err)
{
    ...
    p_tcb->TS = ts;
    *p_err    = OS_ERR_NONE;                                /* Asume we won't have any errors                         */               
    switch (p_tcb->TaskState) {
        case OS_TASK_STATE_RDY:
        case OS_TASK_STATE_DLY:
        case OS_TASK_STATE_SUSPENDED:
