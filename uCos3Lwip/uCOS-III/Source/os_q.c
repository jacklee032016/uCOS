...

{
    ...
    switch (p_q->Type) {
        case OS_OBJ_TYPE_Q:                                 /* Make sure object not already created                   */
...
*                            OS_ERR_OBJ_PTR_NULL          if you pass a NULL pointer for 'p_q'
*                            OS_ERR_OBJ_TYPE              if the message queue was not created
*                            OS_ERR_PEND_ABORT_ISR        If this function was caled from an ISR
                          ...
                          OS_ERR  *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    p_pend_list = &p_q->PendList;
    if (p_pend_list->NbrEntries == (OS_OBJ_QTY)0) {         /* Any task waiting on queue?                             */
        CPU_CRITICAL_EXIT();
...
*                                OS_OPT_POST_NO_SCHED     Do not call the scheduler
*
*                            Note(s): 1) OS_OPT_POST_NO_SCHED can be added (or ORed) with one of the other options.
*                                     2) OS_OPT_POST_ALL      can be added (or ORed) with one of the other options.
...
/*
************************************************************************************************************************
*                                      ADD/REMOVE MESSAGE QUEUE TO/FROM DEBUG TABLE
*
...
{
    if (OSQQty < (OSCfg_QDbgTblSize - 1u)) {    
        OSCfg_QDbgTbl[OSQQty] = p_q;
        p_q->DbgIx            = OSQQty;
    }
}
...
{
    OSCfg_QDbgTbl[p_q->DbgIx] = OSCfg_QDbgTbl[OSQQty];
    OSCfg_QDbgTbl[OSQQty]     = (OS_Q *)0;
    p_q->DbgIx                = OSCfg_QDbgTblSize;
}
#endif
...
void  OS_QInit (OS_ERR *p_err)
{
    ...
    for (i = 0u; i < OSCfg_QDbgTblSize; i++) {
        OSCfg_QDbgTbl[i] = (OS_Q *)0;
    }
#endif
