                   ...
                   OS_ERR              *p_err)
{
    ...
    switch (p_tmr->Type) {
        case OS_OBJ_TYPE_TMR:                               /* Make sure object not already created                   */
...
{
    if (OSTmrQty < (OSCfg_TmrDbgTblSize - 1u)) {    
        OSCfg_TmrDbgTbl[OSTmrQty] = p_tmr;                  /* Add the timer to the debug array                       */
        p_tmr->DbgIx              = OSTmrQty;
    }
}
...
{
    OSCfg_TmrDbgTbl[p_tmr->DbgIx] = OSCfg_TmrDbgTbl[OSTmrQty]; 
    OSCfg_TmrDbgTbl[OSTmrQty]     = (OS_TMR *)0;
    p_tmr->DbgIx                  = OSCfg_TmrDbgTblSize;
}
#endif
...
*                           OS_ERR_PRIO_INVALID          if you specifid the same priority as the idle task
...
void  OS_TmrInit (OS_ERR  *p_err)
{
...
#if OS_CFG_DBG_EN > 0u
    for (i = 0u; i < OSCfg_TmrDbgTblSize; i++) {            /* Initialize the debug array of OS_TMRs                  */
        OSCfg_TmrDbgTbl[i] = (OS_TMR *)0;
    }
#endif
    ...
    OSTaskCreate((OS_TCB    *)&OSTmrTaskTCB,
                 ...
                 (CPU_STK   *)OSCfg_TmrTaskStkLimitPtr,
                 (OS_STK_SIZE)OSCfg_TmrTaskStkSize,
                 (OS_MSG_QTY )0,
                 (OS_TICK    )0,
                 (void      *)0,
                  ...
                  OS_OPT       opt)
{
    ...
    if (p_spoke->NbrEntries > p_spoke->NbrEntriesMax) {     /* Keep track of maximum number of entries in each spoke  */
        p_spoke->NbrEntriesMax = p_spoke->NbrEntries;
    }
    
    p_tmr->PrevPtr = (OS_TMR *)0;                           /* Timer always inserted as first node in list            */
}
