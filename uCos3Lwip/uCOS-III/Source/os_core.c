...
*                                                       EXIT ISR
*
* Description: This function is used to notify uC/OS-III that you have completed serviving an ISR.  When the last nested 
...
#if OS_CFG_SCHED_ROUND_ROBIN_EN > 0u
    if (OSTCBHighRdyPtr->TimeQuanta == (OS_TICK)0) {        /* Load time quanta                                       */
        OSTCBHighRdyPtr->TimeQuantaCtr = OSSchedRoundRobinDfltTimeQuanta;
    } else {
        OSTCBHighRdyPtr->TimeQuantaCtr = OSTCBHighRdyPtr->TimeQuanta;
    }
#endif    
...
void  OSSched (void)
{
    ...
    if (OSTCBHighRdyPtr == OSTCBCurPtr) {                   /* Current task is still highest priority task?           */
        CPU_CRITICAL_EXIT();                                /* Yes ... no need to context switch                      */
        return;
    }
...
*
* Arguments  : en                determines whether roun-robin will be enabled (when DEF_TRUE) or not (when DEF_FALSE)
*
...
*                 c) Load the context of the task pointed to by OSTCBHighRdyPtr.
*                 d) Execute the task.
*
...
*              uC/OS-III's version number multiplied by 1000.  In other words, version 3.00 would be returned as 3000.
*
...
void  OS_IdleTask (void *p_arg)
{
    ...
    OSSchedLockTimeDelta  = (CPU_TS        )0;
    OSSchedLockTimeEnd    = (CPU_TS        )0;
    OSSchedLockTimeOvrhd  = (CPU_TS        )0;
    OSSchedLockTimeMax    = (CPU_TS        )0;
    OSSchedLockTimeMaxCur = (CPU_TS        )0;
    OSSchedLockNestingCtr = (OS_NESTING_CTR)1;              /* Fake locking out the scheduler                        */
    OS_SchedLockTimeMeasStart();                            /* Measure the overhead of the measurement               */
    OSSchedLockNestingCtr = (OS_NESTING_CTR)0;
    OS_SchedLockTimeMeasStop();
    OSSchedLockTimeOvrhd  = OSSchedLockTimeMax;
    OSSchedLockTimeMax    = (CPU_TS        )0;
#endif
...
void  OS_IdleTaskInit (OS_ERR  *p_err)
{
    ...
    OSTaskCreate((OS_TCB    *)&OSIdleTaskTCB,
                 ...
                 (CPU_STK   *)OSCfg_IdleTaskStkLimitPtr,
                 (OS_STK_SIZE)OSCfg_IdleTaskStkSize,
                 (OS_MSG_QTY )0,
                 (OS_TICK    )0,
                 (void      *)0,
                          ...
                          OS_PEND_DATA  *p_pend_data)
{
    ...
    if (p_pend_data->NextPtr == (OS_PEND_DATA *)0) {        /* See if entry is at the tail of the list                */
        p_pend_data_prev          = p_pend_data->PrevPtr;   /* Yes                                                    */
        p_pend_list->TailPtr      = p_pend_data_prev;
    } else if (p_pend_data->PrevPtr == (OS_PEND_DATA *)0) { /* See if entry is at the head of the list                */
        p_pend_data_next          = p_pend_data->NextPtr;   /* Yes                                                    */
        p_pend_list->HeadPtr      = p_pend_data_next;
        p_pend_data_next->PrevPtr = (OS_PEND_DATA *)0;
    } else {
        p_pend_data_prev          = p_pend_data->PrevPtr;   /* Remove from inside the list                            */
        p_pend_data_next          = p_pend_data->NextPtr;
        p_pend_data_prev->NextPtr = p_pend_data_next;
        p_pend_data_next->PrevPtr = p_pend_data_prev;
    }
...
*
*
*                                           .PendDataPtr   .RdyObjPtr     .RdyMsgPtr     .RdyMsgSize    .RdyTS
*                                         +--------------+--------------+--------------+--------------+--------------+
...
*              the tasks that are ready to run.  The list is actually an array os OS_RDY_LIST.  An OS_RDY_LIST contains
...
void  OS_RdyListInsertHead (OS_TCB  *p_tcb)
{
    ...
    if (p_rdy_list->NbrEntries == (OS_OBJ_QTY)0) {          /* CASE 0: Insert when there are no entries               */
        ...
        p_tcb->NextPtr         = p_rdy_list->HeadPtr;       /*         Adjust new OS_TCB's links                      */
        p_tcb->PrevPtr         = (OS_TCB    *)0;
...
void  OS_RdyListInsertTail (OS_TCB  *p_tcb)
{
    ...
    if (p_rdy_list->NbrEntries == (OS_OBJ_QTY)0) {          /* CASE 0: Insert when there are no entries               */
        ...
        p_rdy_list->NbrEntries++;                           /*         One more OS_TCB in the list                    */
        p_tcb->NextPtr          = (OS_TCB   *)0;            /*         Adjust new OS_TCB's links                      */
...
void  OS_SchedLockTimeMeasStop (void)
{
    ...
    if (OSSchedLockNestingCtr == (OS_NESTING_CTR)0) {       /* Make sure we fully un-nested scheduler lock            */
        OSSchedLockTimeEnd = OS_TS_GET();                   /* Get the current time stamp                             */
        delta              = OSSchedLockTimeEnd             /* Compute the delta time between begin and end           */        
                           - OSSchedLockTimeBegin;
        if (delta > OSSchedLockTimeOvrhd) {                 /* Account for overhead                                   */
            OSSchedLockTimeDelta  = delta - OSSchedLockTimeOvrhd;
        } else {
            OSSchedLockTimeDelta  = delta;
        }
        if (OSSchedLockTimeDelta > OSSchedLockTimeMax) {    /* Detect peak value                                      */
            OSSchedLockTimeMax    = OSSchedLockTimeDelta; 
        }
        if (OSSchedLockTimeDelta > OSSchedLockTimeMaxCur) { /* Detect peak value (for resetable value)                */
            OSSchedLockTimeMaxCur = OSSchedLockTimeDelta; 
        }
    }                                                
}                                                     
#endif                                                
...
*
*
* Arguments  : p_rdy    is a pointer to the OS_RDY_LIST entry of the ready list at the current priority
*              -----
*
* Returns    : none
*
