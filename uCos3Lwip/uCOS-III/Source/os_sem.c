...
*                                OS_ERR_OBJ_TYPE        if 'p_sem' has already been initialized to a different object 
*                                                       type
*
* Returns    : none
************************************************************************************************************************
                   ...
                   OS_ERR      *p_err)
{
    ...
    switch (p_sem->Type) {
        case OS_OBJ_TYPE_SEM:                               /* Make sure object not already created                   */
...
/*
************************************************************************************************************************
*                                        ADD/REMOVE SEMAPHORE TO/FROM DEBUG TABLE
*
* Description: These functions are called by uC/OS-III to add or remove a semaphore to/from a message queue debug table.
*
...
{
    if (OSSemQty < (OSCfg_SemDbgTblSize - 1u)) {    
        OSCfg_SemDbgTbl[OSSemQty] = p_sem;
        p_sem->DbgIx              = OSSemQty;
    }
}
...
{
    OSCfg_SemDbgTbl[p_sem->DbgIx] = OSCfg_SemDbgTbl[OSSemQty];
    OSCfg_SemDbgTbl[OSSemQty]     = (OS_SEM *)0;
    p_sem->DbgIx                  = OSCfg_SemDbgTblSize;
}
#endif
...
void  OS_SemInit (OS_ERR  *p_err)
{
    ...
    for (i = 0u; i < OSCfg_SemDbgTblSize; i++) {
        OSCfg_SemDbgTbl[i] = (OS_SEM *)0;
    }
#endif
