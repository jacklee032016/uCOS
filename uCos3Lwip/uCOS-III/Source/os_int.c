...
void  OS_IntQTaskInit (OS_ERR  *p_err)
{
    ...
    for (i = 0u; i < OSCfg_IntQSize; i++) {               
        ...
        p_int_q->MsgSize = (OS_MSG_SIZE)0;
        p_int_q->Flags   = (OS_FLAGS   )0;
        p_int_q->Opt     = (OS_OPT     )0;
    ...
    OSTaskCreate((OS_TCB    *)&OSIntQTaskTCB,
                 (CPU_CHAR  *)"uC/OS-III ISR Queue Task",
                 (OS_TASK_PTR)OS_IntQTask,
                 (void      *)0,
                 (OS_PRIO    )0,                            /* This task is ALWAYS at priority '0' (i.e. highest)     */
                 (CPU_STK   *)OSCfg_IntQTaskStkBasePtr,
                 (CPU_STK   *)OSCfg_IntQTaskStkLimitPtr,
                 (OS_STK_SIZE)OSCfg_IntQTaskStkSize,
                 (OS_MSG_QTY )0,
                 (OS_TICK    )0,
                 (void      *)0,                                 
