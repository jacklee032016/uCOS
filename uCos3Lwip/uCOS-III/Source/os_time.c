...
*
*              opt       specifies whether 'dly' represents absolute or relative time.  
*
*                             OS_OPT_TIME_DLY    specifies a relative time from the current value of OSTickCtr.
*                             OS_OPT_TIME_MATCH  indicates that 'dly' specifies the value that OSTickCtr must reach 
                 ...
                 OS_ERR   *p_err)
{
    ...
    if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0) {        /* Can't delay when the scheduler is locked               */
    ...
    switch (opt) {
        case OS_OPT_TIME_DLY:
             if (dly == (OS_TICK)0) {                       /* 0 means no delay!                                      */
...
*
*              opt       this argument allows you to specify whether you want to strictly be using valid hours (0..999), 
*                        minutes (0..59), seconds (0..59) and milliseconds (0.999) or allow
*                        any value.
*
*                        OS_OPT_TIME_HMSM_STRICT                Only allow hours        (0...99)
*                                                                          minutes      (0...59)
*                                                                          seconds      (0...59)
*                                                                          milliseconds (0...999)
*                        OS_OPT_TIME_HMSM_NON_STRICT            Only allow hours        (0...9999)
*                                                                          minutes      (0...9999)
*                                                                          seconds      (0...65535)
                     ...
                     OS_ERR     *p_err)
{
...
#if OS_CFG_ARG_CHK_EN > 0u
    if (hours == (CPU_INT16U)0) {                           /* Make sure we didn't specify a 0 delay                  */
        if (minutes == (CPU_INT16U)0) {
            if (seconds == (CPU_INT16U)0) {
                if (milli == (CPU_INT32U)0) {
                    *p_err = OS_ERR_TIME_ZERO_DLY;
                    return;
                }
            }
    ...
    switch (opt) {
        case OS_OPT_TIME_HMSM_STRICT:                       /* Validate arguments to be within range                  */
             if (hours > (CPU_INT16U)99) {
                 *p_err = OS_ERR_TIME_INVALID_HOURS;        
                 return;
             }
             if (minutes > (CPU_INT16U)59) {
                 *p_err = OS_ERR_TIME_INVALID_MINUTES;
                 return;
             }
             if (seconds > (CPU_INT16U)59) {
                 *p_err = OS_ERR_TIME_INVALID_SECONDS;
                 return;
             }
             if (milli  > (CPU_INT32U)999) {
                 *p_err = OS_ERR_TIME_INVALID_MILLISECONDS;
                 return;
             }
             break;
             
        case OS_OPT_TIME_HMSM_NON_STRICT:                   /* Validate arguments to be within range                  */
             if (hours > (CPU_INT16U)999) {
                 *p_err = OS_ERR_TIME_INVALID_HOURS;        
                 return;
             }
             if (minutes > (CPU_INT16U)9999) {
                 *p_err = OS_ERR_TIME_INVALID_MINUTES;
                 return;
             }
             break;
    ...
    ticks     = ((OS_TICK)hours * (OS_TICK)3600 + (OS_TICK)minutes * (OS_TICK)60 + (OS_TICK)seconds) * tick_rate
              + tick_rate * ((OS_TICK)milli + (OS_TICK)500 / tick_rate) / (OS_TICK)1000;
...
*                           OS_ERR_TIME_GET_ISR   If you called this function from an ISR
*
...
*
*                           OS_ERR_NONE           If the call was successful
*                           OS_ERR_TIME_SET_ISR   If you called this function from an ISR
*
* Returns    : none
************************************************************************************************************************
*/
