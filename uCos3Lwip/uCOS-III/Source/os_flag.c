                    ...
                    OS_ERR       *p_err)
{
    ...
    switch (p_grp->Type) {
        case OS_OBJ_TYPE_FLAG:                              /* Make sure object not already created                   */
             *p_err = OS_ERR_OBJ_CREATED;
             return;
             
        case OS_OBJ_TYPE_MUTEX:
        case OS_OBJ_TYPE_Q:
        case OS_OBJ_TYPE_SEM:
        case OS_OBJ_TYPE_TMR:
             *p_err = OS_ERR_OBJ_TYPE;                      /* Must not be any of the other object types either       */
             return;
    ...
    p_grp->TS      = (CPU_TS)0;  
    p_grp->Prio    = -1;                                    /* Revision by Michael Vysotsky */
    OS_PendListInit(&p_grp->PendList);

#if OS_CFG_DBG_EN > 0u
    OS_FlagDbgTblAdd(p_grp);
#endif
    OSFlagQty++;    
                             ...
                             OS_ERR       *p_err)
{
    ...
    CPU_CRITICAL_ENTER();
    p_pend_list = &p_grp->PendList;
    if (p_pend_list->NbrEntries == (OS_OBJ_QTY)0) {         /* Any task waiting on the event flag group?              */
        CPU_CRITICAL_EXIT();
...
{                 
    ...
    p_grp->NamePtr          = (CPU_CHAR *)"?FLAG";          /* Unknown name                                           */
...
void  OS_FlagInit (OS_ERR  *p_err)
{
    ...
    for (i = 0u; i < OSCfg_FlagDbgTblSize; i++) {
        OSCfg_FlagDbgTbl[i] = (OS_FLAG_GRP *)0;
    }
#endif
...
/*
************************************************************************************************************************
*                                    ADD/REMOVE EVENT FLAG GROUP TO/FROM DEBUG TABLE
*
* Description: These functions are called by uC/OS-III to add or remove an event flag group from a memory partition 
*              debug table.
*
...
{
    if (OSFlagQty < (OSCfg_FlagDbgTblSize - 1u)) {    
        OSCfg_FlagDbgTbl[OSFlagQty] = p_grp;
        p_grp->DbgIx                = OSFlagQty;
    }
}    
...
{
    OSCfg_FlagDbgTbl[p_grp->DbgIx]  =  OSCfg_FlagDbgTbl[OSFlagQty];
    OSCfg_FlagDbgTbl[OSFlagQty]     = (OS_FLAG_GRP *)0;
    p_grp->DbgIx                    =  OSCfg_FlagDbgTblSize;
}    
#endif
                       ...
                       OS_ERR       *p_err)
{
    ...
    OS_FLAGS        flags_rdy;
    OS_OPT          mode;
    OS_PEND_DATA   *p_pend_data=NULL;
    OS_PEND_DATA   *p_pend_data_next=NULL;
    OS_PEND_LIST   *p_pend_list=NULL;
    OS_TCB         *p_tcb = NULL;
    CPU_SR_ALLOC();
