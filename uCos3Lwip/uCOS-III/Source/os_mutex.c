                     ...
                     OS_ERR      *p_err)
{
    ...
    switch (p_mutex->Type) {
        case OS_OBJ_TYPE_MUTEX:                             /* Make sure object not already created                   */
                        ...
                        OS_ERR    *p_err)
{
    ...
    switch (opt) {
        ...
        case OS_OPT_DEL_ALWAYS:                                            /* Always delete the mutex                 */              
             ...
             if (p_tcb_owner->Prio != p_mutex->OwnerOriginalPrio) {
                              ...
                              OS_ERR    *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* Not allowed to Pend Abort from an ISR                  */
    ...
    switch (opt) {                                          /* Validate the option                                    */
...
void  OS_MutexClr (OS_MUTEX  *p_mutex)
{
    ...
    p_mutex->NamePtr           = (CPU_CHAR     *)"?MUTEX";
    p_mutex->OwnerTCBPtr       = (OS_TCB       *)0;
    p_mutex->OwnerNestingCtr   = (OS_NESTING_CTR)0;
    p_mutex->TS                = (CPU_TS        )0;
    p_mutex->OwnerOriginalPrio =  OS_CFG_PRIO_MAX;
...
*
* Description: These functions are called by uC/OS-III to add or remove a mutex to/from a memory partition debug table.
*
...
{
    if (OSMutexQty < (OSCfg_MutexDbgTblSize - 1u)) {    
        OSCfg_MutexDbgTbl[OSMutexQty] = p_mutex;
        p_mutex->DbgIx                = OSMutexQty;
    }
}
...
{
    OSCfg_MutexDbgTbl[p_mutex->DbgIx] = OSCfg_MutexDbgTbl[OSMutexQty];
    OSCfg_MutexDbgTbl[OSMutexQty]     = (OS_MUTEX *)0;
    p_mutex->DbgIx                    = OSCfg_MutexDbgTblSize;
}
#endif
...
void  OS_MutexInit (OS_ERR  *p_err)
{
    ...
    for (i = 0u; i < OSCfg_MutexDbgTblSize; i++) {
        OSCfg_MutexDbgTbl[i] = (OS_MUTEX *)0;
    }
#endif
