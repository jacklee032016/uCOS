...
{
#if (OS_CFG_DBG_EN > 0u) && ((OS_CFG_TASK_Q_EN > 0u) || (OS_CFG_Q_EN > 0u))
    OS_TCB      *p_tcb;
    OS_MSG_Q    *p_msg_q;
    OS_Q        *p_q;
    OS_OBJ_QTY   i;
...
#if OS_CFG_DBG_EN > 0u
    for (i = 0u; i < OSTaskQty; i++) {                      /* Reset per-Task statistics                              */
        CPU_CRITICAL_ENTER();
        p_tcb                   = OSCfg_TaskDbgTbl[i];
    ...
    for (i = 0u; i < OSCfg_QDbgTblSize; i++) {              /* Reset message queues statistics                        */
        CPU_CRITICAL_ENTER();
        p_q                    =  OSCfg_QDbgTbl[i];
        p_msg_q                = &p_q->MsgQ;
        p_msg_q->NbrEntriesMax = (OS_MSG_QTY)0;
    ...
    for (;;) {
        ...
        if (OSStatTaskCtrMax > OSStatTaskCtrRun) {
            if (OSStatTaskCtrMax > (OS_TICK)0) {
                OSStatTaskCPUUsage = (OS_CPU_USAGE)((OS_TICK)100 - 100 * OSStatTaskCtrRun / OSStatTaskCtrMax);
            } else {
        ...
        if (cycles_total > (OS_CYCLES)0) {
            for (i = 0u; i < OSTaskQty; i++) {         /* Compute execution time of each task                         */
                OS_CRITICAL_ENTER();
                p_tcb           = OSCfg_TaskDbgTbl[i];
                usage           = (OS_CPU_USAGE)((OS_CYCLES)100 * p_tcb->CyclesTotal / cycles_total);
                if (usage > 100u) {
                    usage           = 100u;
                } else {
...
*                            OS_ERR_STK_SIZE_INVALID  If you didn't sepcify a large enough stack.
...
{                    
    ...
    OSTaskCreate((OS_TCB    *)&OSStatTaskTCB,
                 ...
                 (CPU_STK   *)OSCfg_StatTaskStkLimitPtr,
                 (OS_STK_SIZE)OSCfg_StatTaskStkSize,
                 (OS_MSG_QTY )0,
                 (OS_TICK    )0,
                 (void      *)0,
