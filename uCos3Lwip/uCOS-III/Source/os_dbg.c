...
CPU_INT32U  const  OSDbg_DataSize = sizeof(OSIntNestingCtr)
                                  ...
                                  + sizeof(OSSchedLockTimeDelta)
                                  + sizeof(OSSchedLockTimeEnd)
                                  + sizeof(OSSchedLockTimeOvrhd)
...
void  OS_Dbg_Init (void)
{
    ...
    p_temp = p_temp;                                       /* Prevent compiler warning for 'p_temp' not being used!  */
}
#endif
